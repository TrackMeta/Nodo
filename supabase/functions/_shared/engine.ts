...
export async function ventana24hAbierta(db: SupabaseClient, contactId: string): Promise<boolean> {
  try {
    // SOLO las 24h de servicio, que es lo que hace falta para mandar TEXTO LIBRE.
    //
    // ⚠ NO "arreglar" esto usando `conversations.expira_at` (la MAYOR entre las 24h y el Free
    // Entry Point de 72h del anuncio). Ya se hizo una vez, razonando que durante el FEP el
    // texto libre era "gratis y legal según Meta". NO lo es: las 72h del FEP son sobre el
    // COBRO — durante esa ventana los mensajes no se cobran, pero para mandar texto libre
    // sigue haciendo falta la ventana de 24h abierta. Con las 24h cerradas lo único que entra
    // es una plantilla (que ahí sale gratis).
    //
    // Midiendo 72h esta función decía "sí" cuando no se podía: los avisos de pedido (la guía,
    // "llegó a la agencia"), los recordatorios de adelanto y las secuencias mandaban texto
    // libre que Meta RECHAZA con 131047 → el cliente no recibía su guía, su clave ni el
    // remarketing. Y le pasaba justo a los que vinieron por anuncio, que son los que más
    // caro costaron.
