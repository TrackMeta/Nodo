...
export function stagePickerHtml(current) {
  ...
  return `<div class="cx-stagepick">${EMBUDO_STAGES.map((s) => {
    const on = s.k === cur;
    return `<button type="button" class="cx-stagechip${on ? " on" : ""}" data-stagepick="${s.k}"${on ? ` style="background:${s.color}1c;border-color:${s.color};color:${s.color}"` : ""}><span class="cx-sd" style="background:${s.color}"></span><span class="cx-sl">${esc(s.label)}</span>${on ? `<span class="cx-sck">${icon("check","cxi")}</span>` : ""}</button>`;
  }).join("")}</div>`;
}
...
function pedidoProgresoHtml(o) {
  ...
  pasos.forEach((p, i) => {
    ...
    parts.push(`<div style="display:flex;flex-direction:column;align-items:center;gap:3px;flex:1 1 0;min-width:0">
      <span style="width:14px;height:14px;flex:0 0 auto;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:8px;font-weight:800;${dot}">${isDone ? icon("check","cxi") : ""}</span>
...
export function pedidoResumenHtml(o) {
  ...
  const air = s.aereo ? ` <span class="pd-chip air">${icon("plane","cxi")} Aéreo</span>` : "";
  ...
  const bumpRows = bumps.map((b) => {
    ...
    return `<div class="pd-bump"><span class="pd-bi">${esRegalo ? icon("gift","cxi") : icon("plus","cxi")}</span>` +
...
export const EXTRAS_CSS = `
  ...
  .cxi{width:1.05em;height:1.05em;flex:none;vertical-align:-.15em}
...
export async function openDespachoModal(o, deps) {
  ...
  return new Promise((resolve) => {
    ...
    const pintaFoto = () => {
      const box = q("fotobox");
      box.innerHTML = foto
        ? `${fotoKind === "image" ? `<img src="${esc(foto)}" data-ver/>` : `<div class="dsp-doc">${icon("file","cxi")}</div>`}
           <button type="button" data-f="cambiar">Cambiar</button><button type="button" data-f="quitar">Quitar</button>`
        : `<button type="button" data-f="subir">${icon("camera","cxi")} Subir foto o PDF de la guía</button>`;
...
export function openMetaDetalle(order, capiEvent, contact, deps) {
  ...
  const row = (k, v, ok) => `<div class="mdt-row"><span class="mdt-k">${esc(k)}</span><span class="mdt-v">${v ? esc(String(v)) : `<span style="color:var(--faint,#9ca3af)">—</span>`}${(ok && v) ? ` <span style="color:#14b8a6">${icon("check","cxi")}</span>` : ""}</span></div>`;
  ...
  return new Promise((resolve) => {
    ...
    ov.innerHTML = `<div class="vm-card" role="dialog" aria-label="Detalle de envío a Meta" style="max-width:480px">
      <div class="vm-head">
        <div class="vm-ico" style="background:color-mix(in srgb,#1877F2 16%,transparent);color:#1877F2">${icon("megaphone","cxi")}</div>
      ...
      <div class="vm-body">
        ...
        <div class="vm-note"><span class="vm-note-ic">${icon("lock","cxi")}</span><span>Los datos personales (teléfono, nombre, ciudad) se <b>cifran</b> antes de salir; Meta solo los usa para emparejar la conversión. El <b>ctwa_clid</b> es el id del clic en tu anuncio.</span></div>
      </div>
      <div class="vm-foot">
...
export function openVentaManual(contact, deps) {
  ...
  return new Promise(async (resolve) => {
    ...
    ov.innerHTML = `<div class="vm-card" role="dialog" aria-label="Registrar venta a mano">
      <div class="vm-head">
        <div class="vm-ico">${icon("dollar","cxi")}</div>
        <div class="vm-htx">
      ...
      <div class="vm-body">
        ...
        <div class="vm-block">
          ...
          <button type="button" class="vm-add" id="vmAddExtra">${icon("plus","cxi")} Agregar extra</button>
        </div>

        <div class="vm-block vm-ship" id="vmShip" style="display:none">
          <div class="vm-lbl">Datos de entrega</div>
          <div class="vm-seg" id="vmZona">
            <button type="button" class="vm-segb" data-z="lima">${icon("truck","cxi")} Lima · contraentrega</button>
            <button type="button" class="vm-segb" data-z="provincia">${icon("building","cxi")} Provincia · agencia</button>
          </div>
          <div class="vm-row" style="margin-top:8px">
    ...
    function refreshGiftAndNote() {
      ...
      if (regalos.length) {
        ...
        g("#vmGift").innerHTML = `<span class="vm-gift-ic">${icon("gift","cxi")}</span><span>Incluye de regalo: <b>${regalos.map((r) => esc(r.nombre || "regalo")).join(", ")}</b> — se adjunta y entrega solo.</span>`;
      ...
      g("#vmNote").innerHTML = tieneAd
        ? `<span class="vm-note-ic">${icon("compass","cxi")}</span><span>Este cliente vino de un <b>anuncio de Meta</b>: la venta se atribuye al anuncio (se envía el Purchase si tienes el Pixel + token CAPI cargados).</span>`
...
export async function openEditarPedido(o, deps) {
  ...
  const zonaBadge = zona === "digital"
    ? `<span class="pm-zona lima">${icon("dollar","cxi")} Digital</span>`
    : zona === "provincia"
    ? `<span class="pm-zona prov">${icon("building","cxi")} Provincia · agencia</span>`
    : `<span class="pm-zona lima">${icon("truck","cxi")} Lima · contraentrega</span>`;
  ...
  const estadoFisicoHtml = zona === "digital" ? "" : `
        <div class="pm-sec">
          <div class="pm-sec-h">${icon("flag","cxi")} Estado del pedido</div>
          <select id="eEstadoF">
  ...
  const atrHtml = `
        <div class="pm-sec" id="eAtrSec" ${ejesDe(o.product_id).length ? "" : 'style="display:none"'}>
          <div class="pm-sec-h">${icon("box","cxi")} Variante del producto</div>
  ...
  const prodHtml = catalogo.length ? `
        <div class="pm-sec">
          <div class="pm-sec-h">${icon("box","cxi")} Producto del pedido</div>
          <div class="row2">
  ...
  const bumpRow = (b, i) => {
    ...
    return `<div class="eb-row" data-idx="${i}" style="display:flex;align-items:center;gap:8px;margin-bottom:7px;flex-wrap:wrap">
      <span>${esRegalo ? icon("gift","cxi") : icon("plus","cxi")}</span>
      ...
      <button type="button" class="eb-del" title="Quitar del pedido" style="border:0;background:transparent;color:var(--red,#dc2626);cursor:pointer;line-height:1">${icon("x","cxi")}</button>
  ...
  const bumpNewRowHtml = `<div class="eb-new" style="display:flex;align-items:center;gap:6px;margin-bottom:7px;flex-wrap:wrap">
      ...
      <button type="button" class="ebn-del" title="Quitar" style="border:0;background:transparent;color:var(--red,#dc2626);cursor:pointer;line-height:1">${icon("x","cxi")}</button>
      <div class="ebn-vars" style="flex-basis:100%;display:flex;gap:6px;flex-wrap:wrap;padding-left:2px"></div>
    </div>`;
  const bumpHtml = `
        <div class="pm-sec">
          <div class="pm-sec-h">${icon("plus","cxi")} Ventas extra y regalos</div>
  ...
  return new Promise((resolve) => {
    ...
    ov.innerHTML = `<div class="modal"><div class="pedmodal">
      <div class="pm-head">
        <div class="pm-ic">${icon("edit","cxi")}</div>
      ...
      <div class="pm-body">
        <div class="pm-sec" id="eDestSec" ${zona === "digital" ? 'style="display:none"' : ""}>
          <div class="pm-sec-h">${icon("user","cxi")} Destinatario</div>
          <div class="row2">
        ...
        <div class="pm-sec" id="eDigital" ${zona === "digital" ? "" : 'style="display:none"'}>
          <div class="pm-sec-h">${icon("dollar","cxi")} Pago digital</div>
        ...
        <div class="pm-sec" id="eLima" ${zona !== "lima" ? 'style="display:none"' : ""}>
          <div class="pm-sec-h">${icon("pin","cxi")} Entrega en Lima</div>
        ...
        <div class="pm-sec" id="eProv" ${zona === "provincia" ? "" : 'style="display:none"'}>
          <div class="pm-sec-h">${icon("building","cxi")} Envío por agencia (Shalom)</div>
          <div class="row2">
          ...
          <div class="hint">${sug && !s.destino ? `${icon("bulb","cxi")} <b>Sugerida por el bot</b> — la agencia oficial más parecida a lo que dijo el cliente. Confírmala o cámbiala.<br>` : ""}${s.sede ? `<b>El cliente escribió:</b> “${esc(s.sede)}”` : "La oficina Shalom donde recoge tu cliente. Va en el Excel y en el rótulo."}</div>
  ...
  return new Promise((resolve) => {
    ...
    const fila = (o) => {
      ...
      return `<div class="sel-row on" data-id="${esc(o.id)}" data-q="${esc(q)}">
        <span class="sel-check">${icon("check","cxi")}</span>
        <span class="sel-em">${p.emoji ? esc(p.emoji) : icon("box","cxi")}</span>
  ...
  return new Promise((resolve) => {
    ...
    const fila = (o) => {
      ...
      return `<tr data-id="${esc(o.id)}">
        ...
        <td><button type="button" class="dl-foto${s.guia_foto ? " has" : ""}" data-fid="${esc(o.id)}" title="Adjuntar foto del envío / guía">${s.guia_foto ? icon("check","cxi") : icon("camera","cxi")}</button></td>
      </tr>`;
    };
    ...
    ov.querySelectorAll(".dl-foto").forEach((btn) => {
      btn.onclick = () => {
        ...
        inp.onchange = async () => {
          ...
          fotos[btn.dataset.fid] = up; btn.classList.add("has"); btn.innerHTML = icon("check","cxi"); toast("Foto lista");
        };
        inp.click();
      };
    });
    ...
    ov.querySelector("[data-ocr]").onclick = () => {
      ...
      inp.onchange = async () => {
        ...
        for (let i = 0; i < files.length; i++) {
          ...
          fotos[id] = up; const fb = tr.querySelector(".dl-foto"); if (fb) { fb.classList.add("has"); fb.innerHTML = icon("check","cxi"); }
          tr.classList.remove("bad"); asign++;
        }
        btn.disabled = false; btn.innerHTML = icon("camera","cxi") + " Leer guías de fotos";
